...
class Agent:
    ...
    def warmup(self, nsteps):
        for i in range(nsteps // self.num_cores):
            to_eval = \
                [NNIndividual.random_init(self.NNshape) for _ in range(self.num_cores)]
            bds, fitness = \
                zip(*self.pool.starmap(NNIndividual.eval, [*zip(to_eval, self.envs, [self.random_seed]*self.num_cores)]))
    ...
    def eval_and_gradient(self, genome):
        ...
        noisy_genomes[half_batch:, :] -= self.gradient_estimate_sigma * noise
        to_eval = [NNIndividual(self.NNshape, genome)] + [NNIndividual(self.NNshape, g) for g in noisy_genomes]
        bds, fitness = \
            zip(*self.pool.starmap(NNIndividual.eval, [*zip(to_eval, self.envs[:len(to_eval)], [self.random_seed]*len(to_eval))]))
    ...
    def evolve(self, nsteps):
        ...
        while counter < nsteps:
            ...
            individual_centroid = NNIndividual(self.NNshape, genome_centroid)
            ...
            to_eval = [NNIndividual(self.NNshape, g) for g in cmaes_genomes]
