...
class Agent:
    ...
    def eval(self, random_seed=0, return_transitions=True):
        policy = self.gradient_actors[0]
        env = self.envs[0]
        state = env.reset()
    ...
    def evolve(self, nsteps):
        ...
        while counter < nsteps:
            ...
            bds, fitness, transitions = self.eval()
            ...
            for state, action, reward, next_state, done, bd in transitions:
                self.buffer.store_transition(state, action, reward, next_state, done, bd)
        ...
        self.savename = f"TEST_{self.env_id}_{nsteps}"

    def render_best(self):
        best_idv = NNIndividual(self.Ashape, self.gradient_actors[0].flatten_wb())
    ...
    def from_pickle(cls, env_id, nsteps):
        savename = f"TEST_{env_id}_{nsteps}"
