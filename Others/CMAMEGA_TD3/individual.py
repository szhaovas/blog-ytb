...
class NNIndividual:
    def __init__(self, NNshape, genome):
        '''
        NNshape: (input_dims, layer1_dims, layer2_dims, output_dims)
        genome : 1d array containing weights and biases of all layers
        '''
        self.bds = None
    ...
    @classmethod
    def random_init(cls, NNshape):
        return cls(NNshape, PolicyNetwork(NNshape).flatten_wb())

    @staticmethod
    def eval(idv, env, random_seed, return_transitions=True):
        policy = PolicyNetwork(idv.NNshape, False, idv.genome)
        state = env.reset()
        ...
        while not done:
            ...
            if return_transitions:
                # temporary solution; replay should NOT know env.robot.feet_contact
                transitions.append([
                    state,
                    action,
                    reward,
                    ...
                    copy.deepcopy(env.robot.feet_contact)
                ])
            state = new_state

        if return_transitions:
            for _,_,_,_,_,contact in transitions:
                contact[contact == 0] = -1
                contact /= (env.T/1000)
        # sumtree needs positive reward
        # TD3 needs balanced positive/negative rewards but it only uses the
        #   rewards in replay buffer so clipping positive is fine here
        return env.desc, max(env.tot_reward, 1), transitions
...
class Individual:
    ...
    def eval(idv, env, random_seed):
        ...
        while not done:
            ...
            state = new_state
        # FIXME: sumtree needs positive reward
        return env.desc, max(env.tot_reward, 1)
